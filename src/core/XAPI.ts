...
export class XAPI extends Listener {
  ...
  public get positions(): TradePosition[] {
    return Object.values(this._positions)
      .filter(
        (t) =>
          t.value !== null &&
          (t.value.position_type === PositionType.limit ||
            t.value.position_type === PositionType.open)
      )
      .map((t) => t.value);
  }
  ...
  constructor({
    ...
    if (
      ...
      Log.info(
        "Logger path is not defined (this means Logger4 will not saving logs)"
      );
    }
    ...
    if (this.account.safe) {
      Log.info(
        "[TRADING DISABLED] tradeTransaction command is disabled in config (safe = true)"
      );
    }
    this.Stream.onConnectionChange((status) => {
      if (status !== ConnectionStatus.CONNECTING) {
        Log.hidden(
          "Stream " +
            (status === ConnectionStatus.CONNECTED ? "open" : "closed"),
          "INFO"
        );
    ...
    this.Socket.listen.getTrades((data, time, transaction) => {
      ...
      if (sent !== null && sent.elapsedMs() < 1000) {
        ...
        data.forEach((t) => {
          if (
            this._positions[t.position] === undefined ||
        ...
        Log.hidden(
          "getTrades transaction (" + transaction.transactionId + ") is ignored"
        );
      }
    });
    ...
    this.Stream.listen.getTrades((t, time) => {
      ...
      if (
        ...
        this.callListener(Listeners.xapi_onPendingPosition, [
          Utils.formatPosition(t),
        ]);
        ...
        if (
          ...
          this.callListener(Listeners.xapi_onDeletePosition, [
            Utils.formatPosition(t),
          ]);
        }
      } else if (
        ...
        if (this._positions[t.position] !== undefined) {
          ...
          if (value) {
            const changes = Utils.getObjectChanges(
              value,
              Utils.formatPosition(t)
            );
            if (Object.keys(changes).length > 0) {
              this.callListener(Listeners.xapi_onChangePosition, [
                Utils.formatPosition(t),
              ]);
            }
          }
        } else {
          this.callListener(Listeners.xapi_onCreatePosition, [
            Utils.formatPosition(t),
          ]);
        }
    ...
    this.Socket.listen.getServerTime((data, time, transaction) => {
      if (
        ...
        const dif = transaction.response.received.getDifference(
          ...
          this.timer.timeout.push(
            setTimeout(() => {
              if (
                ...
                this.Socket.send.getServerTime().catch((e) => {
                  Log.error(
                    "Socket: getServerTime request failed (XAPI.ts:339)"
                  );
                });
              }
            }, 1000)
  ...
  public getAccountCurrencyValue(
    anotherCurrency: RelevantCurrencies
  ): Promise<number> {
    return Currency2Pair[anotherCurrency] === undefined
      ...
      : Promise.all([
          ...
          return (
            values[1].returnData.profit / values[0].returnData.contractSize
          );
        });
  }
  ...
  public onCreatePosition(
    callBack: (position: TradePosition) => void,
    key: string | null = null
  ) {
    return this.addListener(Listeners.xapi_onCreatePosition, callBack, key);
  }

  public onDeletePosition(
    callBack: (position: TradePosition) => void,
    key: string | null = null
  ) {
